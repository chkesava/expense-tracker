  ...
  if (!vault) {
    return (
      <div className="p-20 text-center">
        ...
        <button onClick={() => navigate("/vaults")} className="mt-4 text-primary font-bold">
          Back to Vaults
        </button>
      </div>
    );
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className="p-8 rounded-[2.5rem] border border-border bg-card shadow-xl relative overflow-hidden">
            <div
          ...
          <div className="p-6 rounded-[2rem] border border-border bg-card/50 backdrop-blur-sm">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="w-9 h-9 rounded-full border-2 border-dashed border-border flex items-center justify-center text-muted-foreground hover:border-primary hover:text-primary transition-all"
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black uppercase tracking-widest text-muted-foreground">Vault Activity</h3>
            <button
              onClick={() => {
                setNewTransaction({
                  amount: 0,
              ...
              className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-xl text-xs font-black uppercase tracking-widest shadow-md hover:bg-primary/90"
            >
              <Plus size={14} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-5 rounded-[2rem] border border-border bg-card">
            ...
            <div className="p-5 rounded-[2rem] border border-border bg-card">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-semibold text-muted-foreground">Deposits Count:</span>
                  <span className="font-black text-foreground">{stats.depositCount}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="font-semibold text-muted-foreground">Withdrawals Count:</span>
                  <span className="font-black text-foreground">{stats.withdrawalCount}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="font-semibold text-muted-foreground">Gross Deposited:</span>
                  <span className="font-black text-success"><Amount value={stats.totalDeposited} /></span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="font-semibold text-muted-foreground">Gross Withdrawn:</span>
                  <span className="font-black text-destructive"><Amount value={stats.totalWithdrawn} /></span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {expensesLoading ? (
              ...
              <div className="p-12 text-center rounded-[2rem] bg-muted/40 border border-dashed border-border">
                <Clock className="mx-auto text-muted-foreground mb-4" size={32} />
                <p className="text-muted-foreground font-bold">No contributions to this vault yet.</p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={cn(
                        ...
                        isDeposit ? "bg-success" : "bg-destructive"
                      )}>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <div className={cn("font-black text-sm", isDeposit ? "text-success" : "text-destructive")}>
                          {isDeposit ? "+" : "-"}<Amount value={expense.amount} />
                        </div>
                      </div>
                      <button
                        onClick={() => expense.id && deleteVaultExpense(expense.id)}
                        className="p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-destructive hover:bg-destructive/10"
      ...
      <Modal isOpen={isInviteOpen} onClose={() => setIsInviteOpen(false)} title="Add members">
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Search user</label>
            <div className="flex gap-2">
              <input
                value={inviteQuery}
                onChange={(e) => setInviteQuery(e.target.value)}
                placeholder="Email, username or name"
                className="flex-1 bg-muted/40 border border-border rounded-2xl px-4 py-3 font-bold text-foreground focus:outline-none focus:border-primary"
              />
              <button
                type="button"
                onClick={runInviteSearch}
                disabled={usersSearching || inviteQuery.trim().length < 2}
                className="px-5 py-3 rounded-2xl bg-primary text-primary-foreground font-black disabled:opacity-50 hover:bg-primary/90"
              >
                Search
              </button>
            </div>
          ...
          <div className="space-y-2">
            {inviteResults.length === 0 ? (
              <div className="text-sm font-bold text-muted-foreground">No results yet.</div>
            ) : (
              inviteResults
                ...
                .map((u) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 p-3 rounded-2xl border border-border bg-card/60 hover:bg-card transition-all text-left"
                  >
                    ...
                    <span className="text-xs font-black uppercase tracking-widest text-primary">Add</span>
                  </button>
                ))
            )}
          </div>
      ...
      <Modal isOpen={isAddTransactionOpen} onClose={() => setIsAddTransactionOpen(false)} title="New Contribution">
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-muted/40 border border-border rounded-2xl px-4 py-3 font-bold text-foreground focus:outline-none focus:border-primary"
                placeholder="₹0.00"
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Note / Description</label>
            <input
          ...
          <button
            type="submit"
            className="w-full py-4 bg-primary text-primary-foreground font-black rounded-2xl shadow-xl hover:bg-primary/90 transition-all"
          >
            Save Transaction
          </button>
        </form>
      ...
      <Modal isOpen={isVaultSettingsOpen} onClose={() => setIsVaultSettingsOpen(false)} title="Vault settings">
        <div className="space-y-3 text-sm text-muted-foreground">
          <div className="font-bold text-foreground">Settings config</div>
