...
export const useCategories = () => {
  ...
  const getSubcategories = useCallback(
    (parentIdOrName: string, opts?: { includeHidden?: boolean }) => {
      const parent =
        ...
        categories.find((c) => c.name === parentIdOrName && (c.kind ?? "category") === "category");
  ...
  const addCategory = async (
    ...
    try {
      const parentRef = await addDoc(collection(db, "users", user.uid, "categories"), {
        name: name.trim(),
        kind: "category",
        parentId: null,
        icon: options?.icon || "📦",
  ...
  const addSubcategory = async (parentId: string, name: string) => {
    if (!user || !name.trim() || !parentId) return;
    try {
      await addDoc(collection(db, "users", user.uid, "categories"), {
        name: name.trim(),
        kind: "subcategory",
  ...
  const renameCategory = async (id: string, newName: string, rewriteExpenses = true) => {
    if (!user || !newName.trim()) return;
    ...
    try {
      await updateDoc(doc(db, "users", user.uid, "categories", id), { name: trimmed });
  ...
  const deleteCategory = async (id: string) => {
    ...
    try {
      ...
      await Promise.all(
        children.map((c) => deleteDoc(doc(db, "users", user.uid, "categories", c.id)))
      );
  ...
  const mergeCategories = async (sourceId: string, targetId: string) => {
    ...
    try {
      ...
      const targetSubNames = new Set(targetSubs.map((s) => s.name.toLowerCase()));
      ...
      for (const sub of sourceSubs) {
        ...
        batch.update(doc(db, "users", user.uid, "categories", sub.id), {
          parentId: targetId,
          name,
        });
        ops++;
      ...
      const budgetsSnap = await getDocs(collection(db, "users", user.uid, "categoryBudgets"));
      ...
      batch.delete(doc(db, "users", user.uid, "categories", sourceId));
      ops++;
      await flush();
